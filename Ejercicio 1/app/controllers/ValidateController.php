...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $errorEdad = '';
    if (!preg_match('/[0-9]{0,}/', $_POST['edad'])) {  $errorEdad .= 'Solo debe contener numeros. ';}
    if ($_POST['edad']>105){$errorEdad .= 'Debe ser menor o igual de 105 años. ';}
    if ($_POST['edad']<1){$errorEdad .= 'Debe ser a partir de 1 año. ';}
    if ($errorEdad!=''){
    ...
    $ErrorAltura = '';
    if (!preg_match('/[0-9]{0,}[,.]{0,1}[0-9]{0,}/', $_POST['altura'])) {$ErrorAltura .= 'Debe contener solo numero ';}
    if ($_POST['altura']>3){$ErrorAltura .= 'Debe ser menor a 3 m. ';}
    if ($_POST['altura']<0.5){$ErrorAltura .= 'Debe ser mayor a 0.5 m. ';}
    if ($ErrorAltura!=''){
    ...
    if(empty($horario_enviado)){
        ...
        $ErrorHorarioTurno='';
        if (!preg_match('/.:00|.:15|.:30|.:45/',$_POST['horaturno'])){$ErrorHorarioTurno .= 'Debe terminar con .00/.15/.30/.45. ';}
